...
export default function MainInfo() {
	...
	const fetchParitcipants = async () => {
		const res = await axios.get(process.env.NEXT_PUBLIC_API_URL + "/bets/participants");
		...
		<MainBackground>
			...
			<MainInfoWrapper>
				...
				<motion.div
					...
					style={{ position: "absolute", width: "100%", textAlign: "center", bottom: 73 }}
				>
					<Participants>
						현재 <Bold>{people}명</Bold>이 승부예측에 참여했어요
					</Participants>
					<Participants>
						<Bold>{korea}명</Bold>이 고려대학교, <Bold>{yonsei}명</Bold>이 연세대학교 학생이에요.
					</Participants>
				</motion.div>
			</MainInfoWrapper>
			<motion.div
...
const MainBackground = styled.div`
	...
	min-height: 657px;
	height: 95vh;
	background: linear-gradient(180deg, rgba(18, 18, 18, 0) 0%, rgba(65, 17, 145, 0.56) 100%);
	padding-top: 58px;
	position: relative;
`;
